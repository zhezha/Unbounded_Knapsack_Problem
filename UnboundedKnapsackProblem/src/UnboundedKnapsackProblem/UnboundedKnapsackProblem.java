...
public class UnboundedKnapsackProblem {
    ...
    void solveDP(int[] values, int[] weights, int totalWeight){  
        ...
        // store if an item is put in the sack or not for the best solution
        boolean[][] p = new boolean[n][totalWeight + 1];
        ...
        // i from n - 1 to 0
        // p[i][W] = true <=> C(W) = C(W - wi) + vi
        // next step: p[?][W - wi] = true <=> C(W - wi) = C(W - wi - ?) + v?
        int j = totalWeight;
        while (j >= minWeights){
            for (int i = n - 1; i >=0; i--){
                if (p[i][j]){
    ...
    String[] formatOutput(int[] bestNumEachItem) {
        ...
        String s = Integer.toString(bestTotalWeight) + ", " + Integer.toString(bestTotalValue) + "\r\n";
        list.add(s);
    ...
    List<Item> getItemList(List<String> lines, int totalWeight) {
        ...
        for(int i = 1; i < numItems; i++){
            ...
            Item c = new Item(i-1, s[0], Integer.parseInt(s[1]), Integer.parseInt(s[2]), totalWeight);
            list.add(c);
        }
